                 ...
                 void (*output_p)(uint8_t *, size_t))
{
    ...
    while (true)
    {
        ...
        if (timer_on && TV_DIFF(end, start) >= RTO)
        {
            ...
            memset(rt_buffer, 0, sizeof(packet) + MAX_PAYLOAD);
            packet *rt_pkt = (packet *)rt_buffer;
            int rthead = retrieve_head_packet(&s_buffer, rt_pkt);

            rt_pkt->ack = htons(next_ack);
            rt_pkt->flags = ACK;

            set_parity(rt_pkt); // must be last
            ...
            // set_parity(rt_packet);
            // print_diag(rt_packet, SEND);
            // if (rthead)
            // {
